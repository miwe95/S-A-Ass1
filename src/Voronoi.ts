...
export class Voronoi extends Container {
    ...
    //calculates 20 seedpoints around the impact point, if points are outside the circle, they are deleted
    ...
    //find closest seed points to a give position
    ...
    //Paints the distance field between 1 and 200 in different colors
    ...
    //event handler when hitting the circle with the player
    ...
    //event handler when clicking the circle
    ...
    //moves the circle when hit by the player
    update(_dt: number) {
        if (this.hit)
            this.circle.x += 100 * _dt / 1000;
    }

    //draws a point on a given position
    ...
    //Interpolates between two color strings and return hex color
